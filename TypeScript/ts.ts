function firstEx(message: string): string {
    let newMessage: string = "";
    let previousLetter: string = message[0];
    let countOfSimilarLetters: number = 1;

    for (let positionOfLetter: number = 1; positionOfLetter < message.length; positionOfLetter++) {
...
function secondEx(message: string): number {
    const morse: Map<string, number[]> = new Map([
    ...
    const morseWords: number[][] = Array.from({ length: words.length }, () => [0, 0]); // вектор слов в коде морзе, заполненный нулями

    for (let i: number = 0; i < words.length; i++) { // преобразование слов в сумму тире и точек слова
    ...
    const setOfMorseWords: Set<string> = new Set(); // множество из уникальных слов в морзе
...
function thirdEx(): void {
    let numOfNumbers: number = 0;
    ...
    for (let i: number = 0; i < numOfNumbers; i++) {
        const numInput = prompt("Enter a number:");
        let num: number = numInput ? parseInt(numInput) : 0;
...
function main(): void {
    ...
    let switcher: number = switcherInput ? parseInt(switcherInput) : 0;
